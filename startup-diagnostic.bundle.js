// startup-diagnostic.js
var output = document.querySelector("#output");
var phase = document.querySelector("#phase");
var errorNode = document.querySelector("#error");
var result = { diagnosticVersion: 2, timestamp: (/* @__PURE__ */ new Date()).toISOString(), url: location.href, origin: location.origin, displayMode: matchMedia("(display-mode: standalone)").matches || navigator.standalone === true, startupMarker: null, serviceWorker: null, assets: [], runtimeManifest: null, runtimeAssetIdentity: [], localStorage: null, errors: [] };
var timeout = (label, promise, ms = 4500) => Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new Error(`${label}_timeout`)), ms))]);
var sha256 = async (bytes) => {
  if (!globalThis.crypto?.subtle) return null;
  const hash = await crypto.subtle.digest("SHA-256", bytes);
  return [...new Uint8Array(hash)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
};
var inspectResponse = async (path) => {
  ...
  try {
    ...
    entry.startsWithHtml = /^\s*<!doctype html|^\s*<html/i.test(text);
    entry.suspiciousJavaScript = path.endsWith(".js") && entry.startsWithHtml;
  } catch (error) {
...
var inspect = async () => {
  ...
  try {
    result.startupMarker = JSON.parse(localStorage.getItem("toyRotation.startupDiagnostic") || "null");
    result.localStorage = { available: true };
  } catch (error) {
    result.localStorage = { available: false, error: String(error?.message || error) };
  }
  try {
    ...
    result.serviceWorker = { supported: "serviceWorker" in navigator, error: String(error?.message || error) };
  }
  ...
  if (manifestEntry.ok && !manifestEntry.startsWithHtml) {
    try {
      ...
      result.runtimeManifest.parsed = true;
      result.runtimeManifest.fileCount = manifest.files?.length || 0;
      result.runtimeManifest.target = manifest.target || null;
      ...
      result.runtimeManifest.error = String(error?.message || error);
    }
  }
  phase.textContent = "Inspection complete \u2014 export is ready.";
...
var exportResult = () => {
  ...
  link.download = `toy-rotation-startup-diagnostic-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.json`;
  link.click();
...
inspect().catch((error) => {
  ...
  output.textContent = JSON.stringify(result, null, 2);
  phase.textContent = "Partial diagnostic ready.";
});
