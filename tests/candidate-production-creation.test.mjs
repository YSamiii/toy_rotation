...
assert.equal(pendingCandidateCount(libraryStore.state),0);const [first,second]=await Promise.all([libraryRecognition.confirm('new-toy'),libraryRecognition.confirm('new-toy')]);assert.equal(first.toy.id,second.toy.id);assert.equal(libraryStore.state.toys.length,1);assert.equal(pendingCandidateCount(libraryStore.state),1);assert.equal(libraryStore.state.catalogState.syncMetadata.localCandidates[0].linkedLocalToyId,libraryStore.state.toys[0].id);
const repeat=await libraryRecognition.confirm('new-toy');assert.equal(repeat.toy.id,first.toy.id);assert.equal(pendingCandidateCount(libraryStore.state),1);
...
console.log('candidate production creation: PASS (20 assertions)');
