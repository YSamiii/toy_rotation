...
import { archiveLocalCandidate, localCandidates, pendingCandidateCount, reopenLocalCandidateReview, setLocalCandidateStatus, unarchiveLocalCandidate, visibleCandidates } from './features/local-candidate-queue.js';
import { getCatalogReports, getPendingCatalogReports, getCatalogReportById, startCatalogReportReview, resolveCatalogReport, dismissCatalogReport } from './features/catalog-report-store.js';
import { getNeedsReviewCount } from './features/review-count.js';
...
async function bootstrapBackground() {
  try {
    ...
    // Legacy report transport entries become local review records only after
    // any old attachment has safely moved into IndexedDB. This is local work;
    // it neither waits for nor depends on Remote Governance.
...
function openWishlistItem() {
  ...
  form.onsubmit=async event=>{event.preventDefault();const values=new FormData(form);const file=values.get('image');let imageRef={kind:'placeholder'};if(file?.size)imageRef=await images.savePersonal(await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);}));const snapshot={canonicalKey:canonicalKey(`${values.get('brand')}-${values.get('productName')}`),brand:values.get('brand'),productName:values.get('productName'),categoryCode:values.get('categoryCode'),skillCodes:values.getAll('skillCodes'),imageRef};const known=catalog.resolve(snapshot);let wish;store.update(state=>{wish=normalizeWishlistItem({catalogId:known?.id||null,catalogSnapshot:snapshot,status:'want',priority:values.get('priority'),notes:values.get('notes'),sourceLink:values.get('sourceLink')});state.wishlist.push(wish);},'wishlist-manual-add');if(!known){governance.createLocalCandidate({candidateId:`candidate-${wish.id}`,source:'manual',candidateType:'new_product_candidate',proposedCanonicalKey:snapshot.canonicalKey,brand:snapshot.brand,productName:snapshot.productName,nameEn:snapshot.productName,categoryCode:snapshot.categoryCode,skillCodes:snapshot.skillCodes,playMechanics:[],imageConsent:false,linkedWishlistId:wish.id});void governance.flushOutbox();}dialog.close();};
}
...
function catalogFilterDefaults() { return { query:'', brands:[], categories:[], skills:[], mechanics:[], challenges:[], age:'all', fitCurrent:false }; }
function cloneCatalogFilters(filters) { return { ...filters, brands:[...filters.brands], categories:[...filters.categories], skills:[...filters.skills], mechanics:[...filters.mechanics], challenges:[...filters.challenges] }; }
function catalogFilterCheckboxes(group, values, label, selected = []) { return `<fieldset class="catalog-filter-group"><legend>${label}</legend><div class="catalog-filter-options">${values.map(([value, text]) => `<label><input type="checkbox" name="${group}" value="${escape(value)}" ${selected.includes(value) ? 'checked' : ''}><span>${escape(text)}</span></label>`).join('')}</div></fieldset>`; }
function openCatalogFilterSheet({ filters, brands, skills, mechanics, onReturn }) {
  ...
  dialog.addEventListener('cancel', event => { event.preventDefault(); returnToCatalog(); }, { once:true });
}
function openCatalog({ initialFilters = catalogFilterDefaults() } = {}) {
  ...
  let catalogLimit = 40;
  const renderCatalogRows = () => { const matches=filterCatalogDevelopment(catalogRows(), filters, { childAgeMonths:childAgeMonths(), profile:store.state.profile?.developmentProfile || {}, history:store.state.developmentFeedbackHistory || [] }); const publicVisibleCount=catalog.getPublicVisibleCatalogCount(); dialog.querySelector('#catalog-meta').textContent=t('catalogResultCount',{shown:Math.min(catalogLimit,matches.length),total:matches.length,catalog:publicVisibleCount}); dialog.querySelector('#catalog-rows').innerHTML = matches.slice(0,catalogLimit).map(renderCatalogCard).join('') || `<p class="panel">${t('noCatalogResults')}</p>`; dialog.querySelector('#catalog-load-more').classList.toggle('hidden',catalogLimit>=matches.length); renderCatalogFilterChips(); wireCatalog(dialog); };
  ...
  dialog.querySelector('#catalog-clear').onclick = () => { Object.assign(filters,catalogFilterDefaults()); dialog.querySelector('#catalog-search').value = ''; resetCatalogRows(); };
...
function openCatalogReport(key) {
  ...
  const dialog=openModal(`<form class="form"><header><h2>${t('catalogReportTitle')}</h2><button type="button" data-close>×</button></header><p>${escape(displayName(toy))}</p><label>${t('catalogReportType')}<select name="reportType"><option value="image_wrong">${t('catalogReportType.image_wrong')}</option><option value="duplicate">${t('catalogReportType.duplicate')}</option><option value="name_wrong">${t('catalogReportType.name_wrong')}</option><option value="brand_wrong">${t('catalogReportType.brand_wrong')}</option><option value="sku_wrong">${t('catalogReportType.sku_wrong')}</option><option value="age_wrong">${t('catalogReportType.age_wrong')}</option><option value="category_wrong">${t('catalogReportType.category_wrong')}</option><option value="skills_wrong">${t('catalogReportType.skills_wrong')}</option><option value="mechanism_wrong">${t('catalogReportType.mechanism_wrong')}</option><option value="parent_child_wrong">${t('catalogReportType.parent_child_wrong')}</option><option value="retired">${t('catalogReportType.retired')}</option><option value="other">${t('catalogReportType.other')}</option></select></label><label>${t('catalogReportDescription')}<textarea name="description" maxlength="1200"></textarea></label><label>${t('catalogReportAttachment')}<input name="attachment" type="file" accept="image/jpeg,image/png,image/webp"></label><button class="primary">${t('catalogReportSubmit')}</button><p id="report-status" role="status"></p></form>`);
  dialog.querySelector('form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target);const file=form.get('attachment');let attachmentRef=null;try{if(file?.size){if(file.size>700000){dialog.querySelector('#report-status').textContent=t('catalogReportAttachmentTooLarge');return;}attachmentRef=await images.savePersonal(await fileToDataUrl(file));}await governance.submitReport({canonicalKey:toy.canonicalKey,reportType:form.get('reportType'),description:form.get('description'),attachmentRef,catalogVersion:store.state.catalogState?.syncMetadata?.lastAppliedRemoteCatalogVersion||0,appVersion:window.TOY_ROTATION_CONFIG?.RELEASE||''});dialog.querySelector('#report-status').textContent=t('catalogReportSubmitted');}catch(error){if(attachmentRef)try{await images.removePersonal(attachmentRef);}catch{}const message=messageFor(error?.message||'');dialog.querySelector('#report-status').textContent=message || t('catalogReportFailed');}};
}
...
function openCatalogManager(key,{dialog:workspaceDialog=null,onReturn=null}={}) {
  ...
  dialog.querySelector('#catalog-delete').onclick = async () => { if (!confirm(t('confirmDelete')) || !confirm(t('confirmDeletePermanent'))) return; try { await admin.delete(toy.canonicalKey, toy); form.querySelector('#catalog-admin-error').textContent='Saved'; } catch (error) { dialog.querySelector('#catalog-admin-error').textContent = messageFor(error.message); } };
}
...
async function openManagerDashboard({ repairMessage = '', returnToSettings = false } = {}) {
  ...
  dialog.querySelector('#admin-export-catalog-count-diagnostic').onclick = () => downloadJson(buildCatalogCountDiagnostic({ catalog, build:window.TOY_ROTATION_CONFIG, uiSearchRows:catalog.search({ includeReview:false }).length }), `toy-rotation-catalog-count-diagnostic-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
...
function renderReportedIssuesQueue(dialog,{onBack,onClose}){const rows=getCatalogReports(store.state);dialog.innerHTML=`<section class="sheet" data-report-queue><header><h2>Reported Issues (${getPendingCatalogReports(store.state).length})</h2><button type="button" data-report-back>‹</button><button type="button" data-close>×</button></header><div class="list">${rows.map(report=>`<article class="panel"><b>${escape(report.catalogCanonicalKey||'Catalog item unavailable')}</b><p>${escape(report.issueType)} · ${escape(report.createdAt)} · ${escape(report.status)}</p><button type="button" data-report-open="${escape(report.id)}">${report.status==='pending'?'Start Review':report.status==='reviewing'?'Continue Review':'View Result'}</button></article>`).join('')||'<p>No reported issues.</p>'}</div></section>`;dialog.querySelector('[data-close]').onclick=()=>dialog.close();dialog.querySelector('[data-report-back]').onclick=()=>renderAdminWorkspaceBody(dialog,{onBack,onClose});dialog.querySelector('[data-report-queue]').addEventListener('click',event=>{const button=event.target.closest('[data-report-open]');if(!button||button.disabled)return;button.disabled=true;const report=getCatalogReportById(store.state,button.dataset.reportOpen);if(report?.status==='pending')store.update(state=>startCatalogReportReview(state,report.id),'catalog-report-reviewing');refreshAdminCandidateBadges();renderReportReviewDetail(dialog,button.dataset.reportOpen,{onBack,onClose});});}
...
// Candidate and Report changes both commit through AppStore.  A single
// subscription keeps every open badge surface derived from canonical state,
// independent of remote transport and candidate-only events.
...
async function bindImages(scope = document) {
  ...
  await Promise.all(targets.map(async image => {
    ...
    try {
      ...
      if(!resolvedImage&&replaceCandidateReviewPreviewWithUnavailable(image)){record('unavailable');return;}
      image.src=resolvedImage || './icons/icon-192.png'; record('assigned');
    }
    catch (error) {
      if(replaceCandidateReviewPreviewWithUnavailable(image)){record('resolve_error',error?.message || String(error));return;}
      image.src='./icons/icon-192.png'; record('resolve_error',error?.message || String(error));
    }
  }));
...
function auditFilenameStamp(date = new Date()) { return date.toISOString().slice(0,16).replace(/-/g,'').replace('T','-').replace(':',''); }
