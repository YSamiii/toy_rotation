...
import { CATEGORY_CODES, SKILL_CODES, canonicalKey, normalizeToy } from './data/schema.js';
...
function render() {
  ...
  root.querySelectorAll('[data-draft-confirm]').forEach(button => { button.onclick = () => getRecognition().confirm(button.dataset.draftConfirm); });
...
function renderWishlist() {
  ...
  const rows = filtered.map(({ item, candidate, result }) => {
    ...
    return `<article class="panel wishlist-card"><div class="wishlist-title"><img data-image='${escapedJson(candidate.imageRef || { kind:'placeholder' })}' alt=""><div><h3>${escape(displayName(candidate))}</h3><p>${escape(brandLabel(candidate.brand))}</p>${ageRange}</div><span class="status-chip">${t(`wishlistStatus.${item.status || 'want'}`)}</span></div><section class="priority-summary"><b>${t('purchasePriority')}</b><span class="priority-${result.purchaseImpact.priority}">${t(result.purchaseImpact.priority)}</span></section>${realSummary}<div class="overlap-list"><h4>${t('realSubstitutes')}</h4>${visible.map(renderOverlap).join('') || `<p>${t('noOverlap')}</p>`}</div>${result.relationships.length > visible.length ? `<button data-action="all-overlaps" data-id="${item.id}">${t('viewAll', { count: result.relationships.length })}</button>` : ''}${skillOnly}<div class="actions">${item.status === 'purchased' ? '' : `<button class="primary" data-action="wishlist-purchased" data-id="${item.id}">${t('purchased')}</button>`}<button data-action="wishlist-remove" data-id="${item.id}">${t('removeFromWishlist')}</button></div></article>`;
  });
  ...
  return `<section class="head"><h2>${t('wishlist')}</h2><button class="primary" data-action="catalog">${t('standardCatalog')}</button></section>${renderCompactFilterBar({ id:'wishlist', searchLabel:t('searchWishlist'), placeholder:t('searchWishlistPlaceholder'), filters:wishlistFilters, includeSort:true })}${diagnostic}<section class="list">${rows.join('') || renderEmpty()}</section>`;
}
...
function openRecognitionReview(id) {
  ...
  form.onsubmit=async event=>{event.preventDefault();try{await saveDraft();const result=await getRecognition().confirm(id);dialog.close();if(result?.toy){view='library';render();setTimeout(()=>document.querySelector(`[data-toy-id="${result.toy.id}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}),0);}}catch(error){form.querySelector('.form-error').textContent=messageFor(error.code||error.message||'recognitionFailed');}};
}
