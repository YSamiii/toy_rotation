...
function openCatalogReport(key) {
  ...
  dialog.querySelector('form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target);const file=form.get('attachment');let attachmentRef=null;try{if(file?.size){if(file.size>700000){dialog.querySelector('#report-status').textContent='Attachment must be 700 KB or smaller.';return;}attachmentRef=await images.savePersonal(await fileToDataUrl(file));}await governance.submitReport({canonicalKey:toy.canonicalKey,reportType:form.get('reportType'),description:form.get('description'),attachmentRef,catalogVersion:store.state.catalogState?.syncMetadata?.lastAppliedRemoteCatalogVersion||0,appVersion:window.TOY_ROTATION_CONFIG?.RELEASE||''});dialog.querySelector('#report-status').textContent='Submitted';}catch(error){if(attachmentRef)try{await images.removePersonal(attachmentRef);}catch{}dialog.querySelector('#report-status').textContent=messageFor(error?.message||'Unable to save report.');}};
}
