...
function renderLocalCandidateQueue(dialog, { returnToSettings = false } = {}) {
  ...
  dialog.innerHTML=`<section class="sheet"><header><h2>Local Candidates (${pendingCandidateCount(store.state)})</h2><button data-local-back>‹</button><button data-close>×</button></header><p>Local-first queue. Remote governance sync may remain unavailable.</p><div class="list">${rows.map(row=>`<article class="panel"><b>${escape(row.productName || row.nameEn || row.proposedCanonicalKey)}</b><p>${escape(row.brand)} · ${escape(row.reviewStatus)} · ${escape(row.syncStatus)}</p><p>${row.imageConsent===true&&row.reviewAttachment?'Review image attached':'No private image available'}</p><div class="actions"><button data-local-review="${escape(row.candidateId)}">Reviewing</button><button data-local-approve="${escape(row.candidateId)}">Approve</button><button data-local-link="${escape(row.candidateId)}">Link Existing</button><button data-local-reject="${escape(row.candidateId)}">Reject</button></div></article>`).join('') || '<p>No local candidates.</p>'}</div></section>`;
  dialog.querySelector('[data-close]').onclick=()=>dialog.close();
  dialog.querySelector('[data-local-back]').onclick=()=>openManagerDashboard({returnToSettings});
  for(const [attribute,status] of [['data-local-review','reviewing'],['data-local-approve','approved'],['data-local-link','linked'],['data-local-reject','rejected']])dialog.querySelectorAll(`[${attribute}]`).forEach(button=>button.onclick=()=>{store.update(state=>setLocalCandidateStatus(state,button.getAttribute(attribute),status),`local-candidate-${status}`);renderLocalCandidateQueue(dialog,{returnToSettings});});
}
