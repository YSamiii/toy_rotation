...
function renderLocalCandidateQueue(dialog, { returnToSettings = false } = {}) {
  ...
  dialog.innerHTML=`<section class="sheet" data-local-candidates-root><header><h2>Local Candidates (${pendingCandidateCount(store.state)})</h2><button type="button" data-local-back>‹</button><button type="button" data-close>×</button></header><p>Local-first queue. Remote governance sync may remain unavailable.</p><div class="list">${rows.map(row=>`<article class="panel"><b>${escape(row.productName || row.nameEn || row.proposedCanonicalKey)}</b><p>${escape(row.brand)} · ${escape(row.reviewStatus)} · ${escape(row.syncStatus)}</p><p>${row.imageConsent===true&&row.reviewAttachment?'Review image attached':'No private image available'}</p><div class="actions"><button type="button" data-local-action="reviewing" data-candidate-id="${escape(row.candidateId)}" ${row.reviewStatus==='reviewing'?'disabled':''}>Reviewing</button><button type="button" data-local-action="approved" data-candidate-id="${escape(row.candidateId)}">Approve</button><button type="button" data-local-action="link" data-candidate-id="${escape(row.candidateId)}">Link Existing</button><button type="button" data-local-action="rejected" data-candidate-id="${escape(row.candidateId)}">Reject</button></div></article>`).join('') || '<p>No local candidates.</p>'}</div></section>`;
