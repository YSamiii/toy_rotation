...
function openSettings() {
  ...
  const adminControls=admin.enabled ? `<button type="button" id="restore-diagnostic-export">${t('exportRestoreDiagnostic')}</button><button type="button" id="manager-open">${t('managerDashboard')} <span class="badge">${pendingCandidateCount(store.state)}</span></button><button type="button" id="admin-open">${t('signOut')}</button>` : `<button type="button" id="admin-open">${t('adminMode')}</button>`;
