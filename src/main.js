...
function renderLocalCandidateQueue(dialog, { returnToSettings = false } = {}) {
  ...
  dialog.innerHTML=`<section class="sheet" data-local-candidates-root><header><h2>Local Candidates (${pendingCandidateCount(store.state)})</h2><button type="button" data-local-back>‹</button><button type="button" data-close>×</button></header><p>Local-first queue. Remote governance sync may remain unavailable.</p><div class="list">${rows.map(row=>`<article class="panel"><b>${escape(row.productName || row.nameEn || row.proposedCanonicalKey)}</b><p>${escape(row.brand)} · ${escape(row.reviewStatus)} · ${escape(row.syncStatus)}</p><p>${row.imageConsent===true&&row.reviewAttachment?'Review image attached':'No private image available'}</p><p class="candidate-next-step">${row.reviewStatus==='reviewing' ? 'Review in progress. Next: Approve, Link Existing, or Reject.' : ''}</p><div class="actions"><button type="button" data-local-action="reviewing" data-candidate-id="${escape(row.candidateId)}" ${row.reviewStatus==='reviewing'?'disabled':''}>Start Review</button><button type="button" data-local-action="approved" data-candidate-id="${escape(row.candidateId)}">Approve</button><button type="button" data-local-action="link" data-candidate-id="${escape(row.candidateId)}">Link Existing</button><button type="button" data-local-action="rejected" data-candidate-id="${escape(row.candidateId)}">Reject</button></div></article>`).join('') || '<p>No local candidates.</p>'}</div></section>`;
  dialog.querySelector('[data-close]').onclick=()=>dialog.close();
  dialog.querySelector('[data-local-back]').onclick=()=>openManagerDashboard({returnToSettings});
  dialog.querySelector('[data-local-candidates-root]').addEventListener('click',event=>{const button=event.target.closest('[data-local-action]');if(!button||button.disabled)return;const action=button.dataset.localAction;const status=action==='link'?'linked':action;store.update(state=>setLocalCandidateStatus(state,button.dataset.candidateId,status),`local-candidate-${status}`);refreshAdminCandidateBadges();renderLocalCandidateQueue(dialog,{returnToSettings});});
}
...
function renderAdminWorkspaceBody(dialog,{onBack,onClose}) {
  dialog.innerHTML=`<section class="sheet admin-workspace-root" data-admin-workspace-root><header><h2>${t('managerDashboard')}</h2><button type="button" data-workspace-back>‹</button><button type="button" data-workspace-close>×</button></header><p role="status">Local Candidates: <span data-admin-pending-badge>${pendingCandidateCount(store.state)}</span></p><button type="button" id="workspace-local-candidates" class="primary">Local Candidates (<span data-admin-pending-badge>${pendingCandidateCount(store.state)}</span>)</button><button type="button" id="workspace-governance">Catalog Governance</button></section>`;
